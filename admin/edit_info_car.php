...
if($_GET){
	if ($_GET['status'] == "edit") {
		...
		$sql = "SELECT carinfo.*,carsbrand.brandType FROM tbl_car_info AS carinfo INNER JOIN tbl_cars_brand as carsbrand ON carinfo.idCarsBrand= carsbrand.idCarsBrand WHERE carinfo.idCar=$id ";
		//echo $sql;
		$qry = mysql_query($sql);
		...
		$id_cars_brand = $row['idCarsBrand'];
		$VIN = $row['VIN'];
		$produceYear = $row['produceYear'];
		$price = $row['price'];
		$currency_unit= $row['currency_unit'];
		$door = $row['numberofdoor'];
		$seat = $row['numberSeat'];
		$features= $row['features'];
		$extColor = $row['exteriorColor'];
		$interColor = $row['interiorColor'];
		$useStatus= $row['usagesStatus'];
		$sellingStatus= $row['sellingStatus'];
		$fuelType = $row['fuelType'];
		$fuelTank = $row['fuelTank'];
		$usedMile = $row['usedMile'];
...
$query=mysql_query("UPDATE tbl_car_info SET idCarsBrand='$idCarBrand',VIN='$VIN',produceYear='$produceYear',price='$price',currency_unit='$currency_unit',numberofdoor='$numberofDoor',numberSeat='$numberSeat',features='$features',exteriorColor='$extColor',interiorColor='$interColor',usagesStatus='$usageStatus',sellingStatus='$sellingStatus',fuelType='$fuelType',fuelTank='$fuelTank',usedMile='$usedMile',steering='$steering',enginepower='$enginepower',transmission='$transmission',vehicletype='$vehicletype',description='$description',regdate='$datefull' WHERE idCar='$id'");

if($query){
			  ob_end_clean();
                    header("Location: list_info_car.php?updmsg=Your information is successfully updated.");
...
<form name="carForm" id="carForm" action="edit_info_car.php" method="POST">
...
<div class="clear"></div><br/><br/><br/><br/><br/><br/>
	<div class="field">	
		...
		<div>
					<input type="submit" name="save" class="form-edit" onclick="return validate_edit_info_car();" />
